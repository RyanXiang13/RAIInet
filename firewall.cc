...
bool Firewall::activate(std::unique_ptr<Link> l, std::unique_ptr<Cell> c, const std::vector<std::unique_ptr<Player>> &players)
{
    // verify that
    ...
    if (l->getIsVirus())
    {
        ...
        players[l->getPlayerID()]->addDownloadedLink(std::move(l));
    }
